...
# FIXME: Cleanup text on schname
# str_trim(link_df$schname, side = c("both"))
...
In our analysis, the index will be our dependent variable. Indexes for keywords cannot be compared, so we must standardize the index within each keyword. We will summarize the index for each school by taking the average of the index for the month.
...
# FIXME: Text cleanup on schname - trim and clean
...
We will add some additional variables to better facilitate our analysis. Since we want to see if the scorecard had an effect on a college's index, we will want to compare the effects before and after the scorecard was published.
...
The additional variables we will create are as follows:

- Scorecard_Published: A Categorical variable that will take a value of 1 if the index occured after the scorecard was published, and a value of 0 if the variable occured before the scorecard was published
- High Income: A categorical variable that will take the value of 1 if the school is in the high income group and 0 if it is in the low income group.
...
We want our final data set to have one value for each school per week.
