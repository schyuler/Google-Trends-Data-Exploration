...
Among colleges that predominantly grant bachelor’s degrees, did it result in more student interest in high-earnings colleges relative to low-earnings ones (as proxied by Google searches for keywords associated with those colleges)?
...
list.files() with the pattern option to get a list of the trends_up_to_ files, and then read them in using a for() loop or (recommended) using map from the purrr package. In either case you will probably want to follow up with a bind_rows().
...
file_list <- list.files(path = './raw-data/', pattern = 'trends_up_to_', full.names = TRUE)

# Load the Google trends data
google_trends_raw_df <- file_list %>%
  map(read_csv) %>%
  bind_rows()
...
```{r message = FALSE, warning = FALSE}
# Remove nulls
google_trends_df <- google_trends_raw_df %>%
  na.omit(google_trends_df)
...
We want our final data set to have one value for each school per month. Therefore, for each school and week we will take the average of our standardized index rather than keeping separate values for each keyword.

```{r message = FALSE, warning = FALSE}
# Do our final grouping of our data by college and week
final_df <- final_df %>% 
  group_by(INSTNM, begdate, median_earnings_p10, high_income, scorecard_published, season, winter, spring, summer, fall) %>% 
  summarise(avg_std_index = mean(std_index))
```

```{r}
# Export our final df to a csv to be used in the final write-up
...
## Earnings and Standardized Index

```{r}
# Assess any relationship between median earnings and average standardized index
final_df %>% 
  group_by(median_earnings_p10, season) %>% 
  summarise(average_index = mean(avg_std_index)) %>% 
  ggplot(mapping = aes(x = median_earnings_p10, y = average_index, color = season)) +
  geom_point()
```
...
```{r message = FALSE, warning = FALSE}
# To make groupsings easier, add a season variable to our dataframe
final_df <- final_df %>% 
  mutate(season = ifelse(winter == 1, 'winter', 
                         ifelse(spring == 1, 'spring',
                                ifelse(summer == 1, 'summer', 'fall')))) 
...
```{r message = FALSE, warning = FALSE}
# Look the scatterplot with colors by season
final_df %>% 
  group_by(begdate, high_income, season) %>% 
  summarise(average_index = mean(avg_std_index)) %>% 
...
```{r message = FALSE, warning = FALSE}
# First regression model with income and scorecard publication
reg1 <- lm(avg_std_index ~ high_income + scorecard_published, data = final_df)

# Second regression model including season variables
reg2 <- lm(avg_std_index ~ high_income + scorecard_published + summer, data = final_df)

# Third regression model, adding interaction term between scorecard and high income
reg3 <- lm(avg_std_index ~ high_income + scorecard_published + summer + scorecard_published*high_income, data = final_df)

# Fourth regression model, using fixed effects controlling for differences between seasons
reg4 <- lm(avg_std_index ~ high_income + scorecard_published + scorecard_published*high_income + summer + fall + spring, data = final_df)
